...
def _migrate_simulation_results(conn: DatabaseConnection) -> None:
    ...
    migrations = {
        ...
        "impots_total_horizon": "ALTER TABLE simulation_results ADD COLUMN impots_total_horizon REAL NOT NULL DEFAULT 0",
        "impot_plus_value": "ALTER TABLE simulation_results ADD COLUMN impot_plus_value REAL NOT NULL DEFAULT 0",
        "patrimoine_net_sortie": "ALTER TABLE simulation_results ADD COLUMN patrimoine_net_sortie REAL NOT NULL DEFAULT 0",
        "break_even_year": "ALTER TABLE simulation_results ADD COLUMN break_even_year INTEGER",
        "nb_annees_cashflow_negatif": "ALTER TABLE simulation_results ADD COLUMN nb_annees_cashflow_negatif INTEGER NOT NULL DEFAULT 0",
    }
    for column, statement in migrations.items():
        if column not in columns:
            conn.execute(statement)
