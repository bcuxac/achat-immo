...
class SourcingOrchestrator:
    ...
    def process_url(self, conn: DatabaseConnection, url: str) -> int:
        ...
        active_map = get_active_simulation_map(
            conn,
            candidate.ville,
            self.profile.simulation_fingerprint,
        )
        map_estimate = None
        map_id = None
        if active_map is not None:
            map_id, viability_map = active_map
            map_estimate = estimate_observation(
                viability_map,
                PropertyObservation(
        ...
        if map_estimate is not None:
            save_map_estimate_run(
                ...
                annonce_id=annonce_id,
                map_id=map_id,
                simulation_hash=self.profile.simulation_fingerprint,
